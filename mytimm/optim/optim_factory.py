...
def create_optimizer_v2(
    ...
    if opt_lower == 'sgd' or opt_lower == 'nesterov':
        ...
        optimizer = optim.SGD(parameters, momentum=momentum, nesterov=True, **opt_args)   # filter(lambda p: p.requires_grad, parameters)
    elif opt_lower == 'momentum':
        opt_args.pop('eps', None)
        optimizer = optim.SGD(filter(lambda p: p.requires_grad, parameters), momentum=momentum, nesterov=False, **opt_args)
    elif opt_lower == 'sgdp':
        optimizer = SGDP(filter(lambda p: p.requires_grad, parameters), momentum=momentum, nesterov=True, **opt_args)
